...
pub struct SearchListResponse {
    pub items: Vec<ProductByIdProduct>,
}
...
pub fn perform_search(
    ...
    let products = queries.get_products_by_search_string(&store_ids, &request.search_term);
    ...
    let mut grouped_products: Vec<ProductByIdProduct> = product_groups
        .into_iter()
        .map(|(_, mut group)| {
        ...
        .collect();
    grouped_products.sort_by(| a, b | a.product_name.partial_cmp(&b.product_name).unwrap());
    SearchListResponse { items: grouped_products }
}